...
class exporter(object):
    def __init__(
        ...
        self.env = req.env
        self.generator = Odoo_generator(self.env)
        ...
        if recs:
            ...
            for i in recs.read(fields):
                ...
                yield '<resource name=%s maximum="%s"><location name=%s/>%s</resource>\n' % (
                    quoteattr(name),
                    i["capacity"] if i["capacity"] is not None else 0,
                ...
                if tmpl["purchase_ok"]:
                    ...
                    for sup in results:
                        ...
                        if sup.get("is_subcontractor", False):
                            ...
                            if sup["standard_packaging_qty"] and (
                                not r["standard_packaging_qty"]
                                or sup["standard_packaging_qty"]
                                < r["standard_packaging_qty"]
                            ):
                                r["standard_packaging_qty"] = sup[
                                    "standard_packaging_qty"
                                ]
                            if sup["price"] and (
                            ...
                            suppliers[(name, sup["date_start"])] = {
                                ...
                                "priority": priority,
                                "standard_packaging_qty": sup["standard_packaging_qty"]
                                or 1,
                            }
                            priority += 1
                    if suppliers:
                        ...
                        for k, v in suppliers.items():
                            yield '<itemsupplier leadtime="P%dD" priority="%s" size_minimum="%f" %s cost="%f"%s%s><supplier name=%s/></itemsupplier>\n' % (
                                v["delay"],
                                v["priority"],
                                v["min_qty"],
                                (
                                    (
                                        ' size_multiple="%f"'
                                        % v["standard_packaging_qty"]
                                    )
                                    if v["standard_packaging_qty"]
                                    else ""
                                ),
                                max(0, v["price"]),
                                (
    ...
    def export_salesorders(self):
        ...
        fields = [
            ...
            "product_uom",
            "order_id",
            "x_studio_confirmed_delivery_date",
            "x_studio_cst_requested_date",
        ]
        ...
        for i in so_line:
            ...
            if not customer or not location or not product:
                ...
                continue
            due = (
                i["x_studio_confirmed_delivery_date"]
                or i["x_studio_cst_requested_date"]
            ...
            yield '<demand name=%s batch=%s quantity="%s" due="%s" priority="%s" minshipment="%s" status="%s"><item name=%s/><customer name=%s/><location name=%s/></demand>\n' % (
                ...
                priority,
                j["picking_policy"] == "one" and qty or 0,
                status,
                quoteattr(product["name"]),
                quoteattr(customer),
                quoteattr(location),
    ...
    def export_purchaseorders(self):
        """
        Send all open purchase orders to frePPLe, using the custom pdp models.

        Only lines in state 'confirmed' are extracted.
        ...
        po_line = [i for i in recs.read(fields) if i["po_id"]]

        # Get all purchase orders
        m = self.env["purchase.order"]
        ids = [i["po_id"][0] for i in po_line if i["po_id"]]
        ...
        for i in po_line:
            ...
            if j["state"] == "done":
                continue
            location = self.mfg_location
            if location and item and i["qty_to_receive"] > i["received_qty"]:
                start = str(
                    timezone("UTC")
                    .localize(j["date_order"])
                    .astimezone(timezone("Asia/Hong_Kong"))
                ).replace(" ", "T")
                end = str(
                    timezone("UTC")
                    .localize((i["confirmed_date"] or i["requested_date"]))
                    .astimezone(timezone("Asia/Hong_Kong"))
                ).replace(" ", "T")
                qty = i["qty_to_receive"] - i["received_qty"]
                yield '<operationplan reference=%s ordertype="PO" start="%s" end="%s" quantity="%f" %s status="confirmed">' "<item name=%s/><location name=%s/><supplier name=%s/>" % (
                    quoteattr("%s - %s" % (j["name"], i["id"])),
                    start,
                    end,
                    qty,
                    ("batch=%s" % quoteattr(i["so_id"][1])) if i["so_id"] else "",
                    quoteattr(item["name"]),
                    quoteattr(location),
        ...
        for i in recs.read(fields):
            if i["bom_id"]:
                ...
                try:
                    startdate = str(
                        timezone("UTC")
                        .localize((i["date_start"] or i["date_planned_start"]))
                        .astimezone(timezone("Asia/Hong_Kong"))
                    ).replace(" ", "T")
                except Exception:
                    continue
                ...
                yield '<operationplan type="MO" reference=%s start="%s" quantity="%s" status="confirmed"%s><operation name=%s/></operationplan>\n' % (
                    quoteattr(i["name"]),
                    startdate,
                    qty,
                    (
                        (" batch=%s" % quoteattr(i["mh_tracking_no"]))
                        if i["mh_tracking_no"]
                        else ""
                    ),
                    quoteattr(operation),
    ...
    def export_onhand(self):
        ...
        self.env.cr.execute(
            """
            select product_id, location_id, sum(quantity), batch from (
            select product_id, location_id, sum(quantity-reserved_quantity) quantity, 'anonymous' as batch from stock_quant
            where location_id in
            (select id from stock_location where active=true and scrap_location=false
            and parent_path not like '3/%'
            and parent_path not like '2/%')
            group by product_id, location_id
            having sum(quantity-reserved_quantity) > 0
            union all
            select stock_move_line.product_id, stock_move_line.location_id,
			sum(stock_move_line.product_uom_qty),
			coalesce(mrp_production.mh_tracking_no,stock_move_line.mh_tracking_no, 'anonymous') batch
			from stock_move_line
			left outer join mrp_production on mrp_production.name = stock_move_line.reference
			where stock_move_line.state in ('assigned','partially_available')
			and stock_move_line.location_id in
            (select id from stock_location where active=true and scrap_location=false
            and parent_path not like '3/%'
            and parent_path not like '2/%')
			group by
			...
			coalesce(mrp_production.mh_tracking_no,stock_move_line.mh_tracking_no,'anonymous')
            ) t
            group by product_id, location_id, batch
            """
        )
        inventory = {}
        for i in self.env.cr.fetchall():
            ...
            if item and location:
                inventory[(item["name"], location, i[3])] = i[2] + inventory.get(
                    (item["name"], location, i[3]), 0
                )

        for key, val in inventory.items():
            if key[2] != "anonymous":
                buf = "%s @ %s @ %s" % (key[0], key[2], key[1])
            else:
                buf = "%s @ %s" % (key[0], key[1])
            yield '<buffer name=%s onhand="%f"%s><item name=%s/><location name=%s/></buffer>\n' % (
                quoteattr(buf),
                val,
                "" if key[2] == "anonymous" else (" batch=%s" % quoteattr(key[2])),
                quoteattr(key[0]),
                quoteattr(key[1]),
            )
